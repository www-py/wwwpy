...
def test_nested():
    ...
    node_list = CstNodeList([CstNode(tag_name='p', span=(5, 12), attr_span=(7, 7), content_span=(8, 8))])
...
def test_nested_with_attributes_and_spaces():
    ...
    children = CstNodeList([CstNode(tag_name='p', span=(19, 27), attr_span=(21, 22), content_span=(23, 23))])
...
def test_void_tags_with_attributes_and_spaces():
    ...
    children = CstNodeList([CstNode(tag_name='input', span=(6, 28), attr_span=(12, 12 + 15), attributes_list=attrs)])
...
def _clean(tree: CstNodeList) -> CstNodeList:
    for node in tree:
        node.parent = None
        node.level = 0
